...
```sql
-- 관리자 판별: admins 테이블에 이 사용자의 UUID 가 있는지 본다.
-- (초기엔 이메일 문자열로 판별했으나, 이메일 인증이 꺼져 있어 계정 선점에
--  취약해서 004_review_fixes.sql 에서 UUID 등록 방식으로 바꿨다)
...
상태 전이에 `PROCESSING`을 둔 이유: 결제 확인 창을 두 번 열면 두 요청이 모두 주문을 `PENDING`으로 읽는다. 예전 코드는 하나가 `PAID`로 만든 뒤 다른 하나가 토스 거부를 받아 `FAILED`로 덮어써, **결제됐는데 실패로 보이는** 상태가 됐다. 지금은 `PENDING → PROCESSING` 조건부 UPDATE로 한 요청만 통과시키고(선점), 이후 실패 기록도 `PROCESSING`인 행만 건드려 `PAID`를 덮지 않는다.
...
④ confirm-payment (Edge Function)
   ...
   4. 상태 확인            PAID면 성공 반환 / PROCESSING이면 409
   5. 주문 선점            PENDING → PROCESSING (조건부 UPDATE)  ← 동시 승인 차단
                          선점 실패(=다른 요청이 이미 처리 중)면 409
   6. 금액·재고 대조       price × quantity ≠ amount 면 400, 재고 부족이면 409
                                              ↑ 가장 중요한 방어선
   7. 토스 승인 API 호출   POST https://api.tosspayments.com/v1/payments/confirm
                          Authorization: Basic base64(시크릿키 + ":")
   8. 결과 기록            성공 → PAID / 실패 → FAILED (PROCESSING인 행만)
        │
⑤ orders.html 에서 확인
```
...
| 항목 | 현재 상태 |
|---|---|
| 재고 차감 | 결제해도 `stock`이 안 줄어든다 (단, `stock`이 0이면 결제는 막힌다) |
| 환불 | 미구현 |
| 장바구니 | 없음. 1상품 1개 고정 |
